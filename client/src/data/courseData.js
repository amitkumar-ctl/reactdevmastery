...
export const CONCEPTS = {
  ...
  'devtools': {
    title: 'DevTools & Debugging', color: '#06b6d4',
    items: [
      { id: 'devtools-elements',     title: 'Elements & Styles Panel',     desc: 'DOM inspection, live CSS editing, computed styles, box model, and forced states.',          diff: 'easy'   },
      { id: 'devtools-console',      title: 'Console API Mastery',         desc: 'Beyond console.log — table, group, time, trace, assert, and conditional breakpoints.',      diff: 'easy'   },
      { id: 'devtools-network',      title: 'Network Panel Deep Dive',     desc: 'Waterfall analysis, throttling, XHR replay, response inspection, and HAR exports.',         diff: 'medium' },
      { id: 'devtools-performance',  title: 'Performance Panel',           desc: 'Flame charts, long tasks, layout thrashing, FPS drops, and scripting cost.',               diff: 'hard'   },
      { id: 'devtools-memory',       title: 'Memory Panel',                desc: 'Heap snapshots, allocation timelines, detached nodes, and finding memory leaks.',           diff: 'hard'   },
      { id: 'devtools-sources',      title: 'Sources & Breakpoints',       desc: 'Debugger, conditional breakpoints, logpoints, watch expressions, and call stack.',          diff: 'medium' },
    ],
  },
...
// ── ADD TO CONCEPTS OBJECT ────────────────────────────────────────────
  ...
  'prototypes': {
    title: 'Prototypes & OOP', color: '#a855f7',
    items: [
      { id: 'proto-chain',      title: 'Prototype Chain',            desc: 'How JS resolves property lookups via [[Prototype]] — the engine behind all inheritance.',         diff: 'hard'   },
      { id: 'proto-create',     title: 'Object.create & new',        desc: 'Differences between Object.create(), new Constructor(), and class — what each sets up under the hood.', diff: 'hard'   },
      { id: 'proto-class',      title: 'ES6 Classes Deep Dive',      desc: 'class syntax, extends, super, static methods, private fields — and the prototype truth beneath.',  diff: 'medium' },
      { id: 'proto-inherit',    title: 'Inheritance & Composition',  desc: 'Prototypal vs classical inheritance, mixins, and why composition beats inheritance.',              diff: 'hard'   },
      { id: 'proto-instanceof', title: 'instanceof & Type Checking', desc: 'How instanceof walks the chain, Symbol.hasInstance, and reliable type-checking alternatives.',     diff: 'medium' },
    ],
  },
...
'si-oauth': [
  ...
  // ── Prototypes & OOP ──────────────────────────────────────────────────
  'proto-chain': [
    { q: "What is the prototype chain?", opts: ["A linked list of functions", "A chain of objects linked via [[Prototype]] — property lookup walks up the chain until found or null is reached", "The call stack order", "CSS selector specificity"], correct: 1, explain: "Every object has an internal [[Prototype]] slot. When you access obj.prop, JS looks on the object first, then follows [[Prototype]] to the next object, all the way up to Object.prototype, then null. This is how toString() is available on every object." },
    { q: "What does this log?\nconst a = { x: 1 };\nconst b = Object.create(a);\nconsole.log(b.x);", opts: ["undefined", "1", "null", "ReferenceError"], correct: 1, explain: "b's [[Prototype]] is a. b has no own property x, so JS looks up the chain and finds x:1 on a. Result: 1. This is prototype-based inheritance in its purest form." },
    { q: "How do you check if a property is OWN vs inherited?", opts: ["typeof obj.prop", "obj.hasOwnProperty('prop') or Object.hasOwn(obj, 'prop')", "obj.prop !== undefined", "Object.keys(obj)"], correct: 1, explain: "hasOwnProperty returns true only for properties directly on the object. Object.hasOwn(obj, key) is the modern version. Object.keys() also returns only own enumerable properties." },
    { q: "What is Object.prototype?", opts: ["The base class for all objects", "The top of every prototype chain — provides toString(), hasOwnProperty(), valueOf() to all objects", "A special function", "The global object"], correct: 1, explain: "Object.prototype is the final link before null in every prototype chain. Object.create(null) creates an object WITH NO prototype — no toString, no hasOwnProperty. Useful for pure hash maps." },
    { q: "Should you use __proto__ to set prototypes?", opts: ["Yes, it's the standard way", "No — use Object.getPrototypeOf() to read, Object.create() to create with a prototype, Object.setPrototypeOf() to change", "Only in classes", "Only in Node.js"], correct: 1, explain: "__proto__ is a legacy accessor. Setting it at runtime can break V8 optimizations. Prefer Object.getPrototypeOf() to read, Object.create(proto) to create, Object.setPrototypeOf(obj, proto) to change." },
  ],

  'proto-create': [
    { q: "What does 'new Constructor()' do step by step?", opts: ["Just calls the function", "1) Creates empty object, 2) Sets [[Prototype]] to Constructor.prototype, 3) Runs Constructor with this = new object, 4) Returns the object", "Copies properties from Constructor", "Calls Object.create()"], correct: 1, explain: "new Dog('Rex'): JS creates {}, sets {}.__proto__ = Dog.prototype, runs Dog() with this pointing to the new object, then returns it. If the constructor explicitly returns an object, that object is used instead." },
    { q: "What is the difference between Object.create(proto) and new Constructor()?", opts: ["They are identical", "Object.create: sets prototype without calling any constructor. new: calls constructor AND sets prototype.", "new is faster", "Object.create is deprecated"], correct: 1, explain: "Object.create(Animal.prototype) creates an object inheriting from Animal.prototype but DOESN'T call Animal(). No constructor logic runs. new Animal() does both — sets the prototype AND runs the constructor." },
    { q: "What does Object.create(null) create?", opts: ["An empty array", "An object with NO prototype — no toString, no hasOwnProperty, no inherited anything", "A null value", "An object equal to {}"], correct: 1, explain: "Object.create(null): [[Prototype]] is null — no inherited properties. Safer as a dictionary/hash map: no accidental 'constructor' or '__proto__' key collisions. Used in Lodash, Vue reactivity internals." },
    { q: "What breaks when you replace Dog.prototype = {} entirely?", opts: ["Nothing breaks", "Dog.prototype.constructor becomes Object instead of Dog — the back-reference is lost", "All methods are deleted", "new Dog() stops working"], correct: 1, explain: "Dog.prototype.constructor === Dog by default. Replace the whole prototype object and that reference is gone. Fix: Dog.prototype = { constructor: Dog, ...methods } — restore it explicitly. Or add methods individually: Dog.prototype.bark = function(){}" },
  ],

  'proto-class': [
    { q: "What does typeof MyClass return for an ES6 class?", opts: ["'class'", "'function' — classes are syntactic sugar over constructor functions", "'object'", "'undefined'"], correct: 1, explain: "typeof MyClass === 'function'. Classes ARE functions. class Animal { speak() {} } compiles to function Animal() {} + Animal.prototype.speak = function(){}. The runtime behaviour is identical to ES5 prototype patterns." },
    { q: "What does super() do in a class constructor?", opts: ["Calls a static method", "Calls the parent constructor — MUST be called before accessing 'this' in derived class", "Creates a super-object", "Accesses parent prototype"], correct: 1, explain: "In a derived class, 'this' doesn't exist until super() runs. Forgetting super() = ReferenceError: 'Must call super constructor before accessing this'. super() creates and initialises the object via the parent constructor." },
    { q: "class Btn { handleClick() {} } vs class Btn { handleClick = () => {} } — what is the difference?", opts: ["No difference", "Method: on prototype, shared by all instances. Arrow field: created per-instance, each has own copy but 'this' is always correct.", "Arrow fields are faster", "Methods don't support 'this'"], correct: 1, explain: "Method on prototype: one function shared by all instances. Arrow class field: a NEW function created for EACH instance in the constructor. Arrow fields fix 'this' binding but cost more memory — one function per instance instead of one shared." },
    { q: "What do private class fields (#) guarantee?", opts: ["Fields are read-only", "Truly inaccessible outside the class — enforced by the JS engine, not a naming convention", "Fields are faster", "Same as TypeScript private"], correct: 1, explain: "class User { #password } — accessing user.#password outside the class throws SyntaxError. Enforced by the engine, not just convention like _password. Private fields don't appear in Object.keys(), JSON.stringify(), or any reflection." },
    { q: "What does 'static' mean on a class method?", opts: ["It never changes", "Belongs to the CLASS itself, not instances — called as ClassName.method(), not instance.method()", "Makes property read-only", "Shares value across modules"], correct: 1, explain: "class MathUtils { static add(a, b) { return a+b; } } — MathUtils.add(1,2) works. new MathUtils().add is undefined. Static methods are on the constructor function itself, not on the prototype." },
  ],

  'proto-inherit': [
    { q: "What is 'composition over inheritance'?", opts: ["Using CSS composition", "Building objects by combining small focused behaviours instead of deep inheritance chains — more flexible, avoids fragile base class problem", "React composition only", "A performance optimization"], correct: 1, explain: "Deep inheritance: change Animal and everything breaks. Composition: const dog = Object.assign({}, canBark, canSwim) — mix exactly what you need. No hierarchy to break. React hooks embody this: combine useFetch + useDebounce without any inheritance." },
    { q: "What is the Gorilla-Banana problem?", opts: ["A memory leak", "You asked for a banana (one method) but got the gorilla (entire class hierarchy) and the jungle (all parent methods)", "A naming collision", "A circular dependency"], correct: 1, explain: "Deep inheritance forces you to take everything up the chain. You want bark() from Dog but you get Animal, Mammal, Pet, Dog — all their state and methods bundled in. Composition lets you take only what you need." },
    { q: "What does Object.assign({}, mixin1, mixin2) do?", opts: ["Creates a class with two parents", "Copies methods from mixin1 and mixin2 into a new object — composition without any prototype chain", "Deep clones the mixins", "Creates a Proxy"], correct: 1, explain: "Object.assign copies own enumerable properties. const dog = Object.assign({name:'Rex'}, canBark, canSwim) — dog gets bark() and swim() directly as own properties. No prototype chain involved. Simple, explicit, flexible." },
    { q: "What is class Dog extends Animal setting up?", opts: ["Copies Animal's methods to Dog", "Dog.prototype.__proto__ = Animal.prototype AND Dog.__proto__ = Animal (static inheritance)", "Creates a copy of Animal", "Runs Animal's constructor"], correct: 1, explain: "extends wires TWO chains: 1) Instance: dog → Dog.prototype → Animal.prototype. 2) Static: Dog → Animal (Dog inherits Animal's static methods). super() in the constructor creates the instance via Animal's constructor." },
  ],

  'proto-instanceof': [
    { q: "How does instanceof actually work?", opts: ["Checks the object's type string", "Walks the prototype chain looking for Constructor.prototype — returns true if found anywhere", "Checks constructor name", "Uses typeof internally"], correct: 1, explain: "dog instanceof Animal: JS checks if Animal.prototype exists anywhere in dog's prototype chain. dog → Dog.prototype → Animal.prototype → found → true. It checks the prototype OBJECT, not the constructor function name." },
    { q: "Why does [] instanceof Array fail across iframes?", opts: ["iframes block JS", "Each frame has its own Array constructor — the iframe's array has iframe.Array.prototype, not main window's Array.prototype", "instanceof is banned in iframes", "Arrays aren't objects"], correct: 1, explain: "Each browsing context has its own global object and built-ins. An array from an iframe fails instanceof Array in the main window because they reference different Array.prototype objects. Fix: Array.isArray() — checks internal [[IsArray]] slot, works everywhere." },
    { q: "What is the most reliable way to check if a value is an array?", opts: ["value instanceof Array", "typeof value === 'array'", "Array.isArray(value) — works across iframes, not fooled by prototype changes", "value.constructor === Array"], correct: 2, explain: "Array.isArray() checks [[IsArray]] slot — works cross-frame, works even if Array.prototype was replaced. instanceof fails cross-frame. typeof returns 'object'. value.constructor can be overwritten. Always use Array.isArray()." },
    { q: "What does Symbol.hasInstance allow?", opts: ["Create custom symbols", "Override how instanceof behaves for a class — static [Symbol.hasInstance](instance) { return customCheck; }", "Check if a symbol exists", "Create private fields"], correct: 1, explain: "class Even { static [Symbol.hasInstance](n) { return n % 2 === 0; } } — 4 instanceof Even is true. 3 instanceof Even is false. Completely overrides normal prototype chain check. Used by built-in types like Function." },
    { q: "What does Object.getPrototypeOf({}) return?", opts: ["null", "Object.prototype — the top of every regular object's chain", "undefined", "The object itself"], correct: 1, explain: "Object.getPrototypeOf({}) === Object.prototype. Object.getPrototypeOf(Object.prototype) === null — chain ends here. Object.getPrototypeOf([]) === Array.prototype. Use this instead of __proto__ to safely inspect the chain." },
  ],
  // ── DevTools & Debugging ──────────────────────────────────────────────
  'devtools-elements': [
    { q: "How do you force a hover/focus/active CSS state in DevTools without holding the mouse?", opts: ["You can't — must use mouse", "Elements panel → right-click element → Force state → :hover/:focus/:active — state persists while you edit CSS", "Add a CSS class manually", "Use the Console to trigger it"], correct: 1, explain: "Right-click any element in Elements panel → Force state → pick :hover, :focus, :active, :focus-within, :focus-visible, :visited. The state is locked so you can freely edit the CSS rules that apply to it. Essential for debugging hover/focus styles." },
    { q: "What does the computed tab show vs the styles tab?", opts: ["They are identical", "Styles: all CSS rules with source file and line. Computed: the final resolved value for every property after cascade/inheritance — no duplicates.", "Computed shows only custom properties", "Styles shows inherited only"], correct: 1, explain: "Styles tab: every matching rule, where it came from, what's overridden (strikethrough). Computed tab: the final winning value for each property. Use Computed to answer 'what is the actual margin?' when many rules compete." },
    { q: "How do you find which element is causing unexpected layout space in DevTools?", opts: ["Guess and check", "Hover elements in the Elements panel — DevTools highlights them with a box model overlay showing margin/border/padding in colour", "Check the Console for errors", "Use the Network tab"], correct: 1, explain: "Hover elements in the Elements tree — the page highlights the element with a coloured overlay: blue = content, green = padding, orange = margin, yellow = border. Instantly see which box model layer is creating unexpected space." },
    { q: "How do you copy a DOM element's full selector path quickly in DevTools?", opts: ["Type it manually", "Right-click element in Elements panel → Copy → Copy selector (or Copy JS path for JS access)", "Use document.querySelector in Console", "Use the Sources panel"], correct: 1, explain: "Right-click element → Copy → options include: Copy selector (CSS path like #main > div.card:nth-child(2)), Copy JS path (document.querySelector(...)), Copy XPath, Copy outerHTML. Copy selector is fastest for CSS debugging." },
  ],

  'devtools-console': [
    { q: "What does console.table() do?", opts: ["Prints a table-like string", "Renders arrays of objects as an interactive sortable table — far more readable than console.log for structured data", "Creates an HTML table", "Only works with arrays"], correct: 1, explain: "console.table([{name:'Alice',age:30},{name:'Bob',age:25}]) renders a proper table with sortable columns. Pass a second argument to filter columns: console.table(users, ['name', 'email']). Game-changer for debugging API responses." },
    { q: "What is the difference between console.log() and console.dir()?", opts: ["They are identical", "console.log: renders HTML elements as DOM tree. console.dir: renders any value as a JS object with expandable properties", "console.dir is deprecated", "console.log is faster"], correct: 1, explain: "console.log(document.body) shows an interactive DOM tree. console.dir(document.body) shows it as a JS object with all properties (tagName, children, style, etc.). Use console.dir when you want to see an element's JS properties, not its HTML structure." },
    { q: "How do you measure execution time with the Console API?", opts: ["Date.now() before and after", "console.time('label') to start, console.timeEnd('label') to stop and print elapsed ms — multiple concurrent timers supported", "performance.now() only", "Use the Performance panel only"], correct: 1, explain: "console.time('fetch'); await fetch('/api'); console.timeEnd('fetch'); — prints 'fetch: 142.3ms'. Multiple named timers run concurrently. console.timeLog('fetch') prints intermediate time without stopping. Quick alternative to the Performance panel." },
    { q: "What is a logpoint and why is it better than adding console.log to source code?", opts: ["A console.log with a condition", "A breakpoint that logs an expression without pausing execution AND without modifying source code — removed when DevTools closes", "A network request log", "A performance marker"], correct: 1, explain: "Sources panel → right-click gutter → Add logpoint → enter expression like 'user:', user. Logs to Console on every pass without modifying your code, without saving, without committing. Perfect for production debugging where you can't modify files." },
    { q: "What does console.assert() do?", opts: ["Throws an error", "Logs an error message ONLY if the first argument is falsy — silent when condition is true. Useful for invariant checking.", "Creates a test assertion", "Pauses execution"], correct: 1, explain: "console.assert(user.id !== null, 'user.id should not be null', user) — if user.id IS null, logs the error + extra args. If it's not null, does nothing. Cleaner than if(!x) console.error() for quick sanity checks." },
  ],

  'devtools-network': [
    { q: "What does the waterfall column in the Network panel show?", opts: ["Download speed only", "Each request's timing phases: queuing, stalled, DNS, connection, SSL, TTFB (waiting), content download — colour coded", "Request size over time", "Server response codes"], correct: 1, explain: "Waterfall bars are colour-coded: grey=queuing/stalled, orange=TTFB, blue=download. Width shows time. Stacked bars (HTTP/1.1) vs parallel (HTTP/2). Long orange bar = slow server. Long blue = large file. Gaps between bars = connection overhead." },
    { q: "How do you simulate a slow mobile connection in DevTools?", opts: ["You can't without real devices", "Network panel → throttle dropdown → select 'Slow 3G', 'Fast 3G', or create a custom profile", "Only available in Lighthouse", "Use the Performance panel"], correct: 1, explain: "Network panel → No throttling dropdown → pick Slow 3G (400kbps, 400ms RTT) or Fast 3G (1.5Mbps, 40ms RTT). Also throttles DNS and connection time. Add CPU throttling in Performance panel to simulate a mid-range Android device." },
    { q: "How do you replay an XHR/fetch request without refreshing the page?", opts: ["You can't replay requests", "Right-click the request in Network panel → Replay XHR — resends the identical request and shows the new response", "Copy the URL and paste in new tab", "Use the Sources panel"], correct: 1, explain: "Right-click any XHR/Fetch request → Replay XHR. Resends with the same method, headers, and body. Useful for testing API changes without re-triggering the full user flow. Note: only works for XHR, not for document/script/image requests." },
    { q: "What does 'Disable cache' in Network panel do and when should you use it?", opts: ["Clears the cache once", "Prevents browser from using cached responses while DevTools is open — every request hits the server fresh", "Disables service workers", "Only affects images"], correct: 1, explain: "Disable cache: checked → browser sends Cache-Control: no-cache on every request, ignores cached responses. Essential when testing caching behaviour or verifying new deployments. Only active while DevTools is open — normal caching resumes when you close DevTools." },
    { q: "How do you find which request is blocking page render?", opts: ["Check all requests manually", "Filter by type → look for render-blocking CSS/JS: they appear in the waterfall starting from the very beginning with a red triangle icon", "Check the Console for errors", "Use Lighthouse only"], correct: 1, explain: "Render-blocking resources appear at the top of the waterfall (requested immediately) and have a vertical red line at their right edge showing when rendering started. CSS and sync scripts are render-blocking. Check their size and consider deferring or inlining critical CSS." },
  ],

  'devtools-performance': [
    { q: "What is a Long Task in the Performance panel?", opts: ["Any task over 1 second", "A JavaScript task that blocks the main thread for more than 50ms — shown with a red triangle. Prevents browser from responding to input.", "A slow network request", "A large paint area"], correct: 1, explain: "50ms threshold: browser must complete all work within 50ms to respond to user input within 100ms (RAIL model). Tasks >50ms = Long Tasks, shown with red triangle in Performance panel. Multiple long tasks cause high TBT (Total Blocking Time) and hurt INP." },
    { q: "How do you read a flame chart to find a bottleneck?", opts: ["Read top to bottom", "Find the widest bar at the deepest level — width = time spent. The bottom-most wide function is the actual bottleneck, everything above is just the call chain.", "Find the tallest stack", "Look for red bars only"], correct: 1, explain: "Flame chart: each row = one level of the call stack. Time flows left to right. Width = time taken. To find bottleneck: look for the widest bar at the lowest level. Everything above it (handleClick → processData → expensiveCalc) is just the path that led there." },
    { q: "What does the Main thread track in the Performance recording show?", opts: ["Network requests", "Every JS task, style recalculation, layout, paint, and composite operation — colour coded by type", "Memory usage", "Frame rate only"], correct: 1, explain: "Colour coding in Main thread: Yellow=scripting (JS), Purple=rendering (layout/style), Green=painting, Grey=system. A wall of yellow = too much JS. Alternating yellow+purple = layout thrashing. Look for the first Long Task after your interaction." },
    { q: "How do you record a Performance profile for a specific interaction?", opts: ["Record the whole session", "Click Record → perform the specific interaction (click, scroll, type) → click Stop → analyse the 1-5 second window", "Use Lighthouse instead", "Set a timer before recording"], correct: 1, explain: "Short focused recordings are easier to analyse. Click Record → immediately perform the slow action → Stop. 1-5 seconds of data. Too long = too much noise. Then zoom into the interaction in the timeline to see exactly what ran during that specific event." },
  ],

  'devtools-memory': [
    { q: "What is the workflow for finding a memory leak with Heap Snapshots?", opts: ["Take one snapshot and look for large objects", "Snapshot 1 (baseline) → interact repeatedly → Snapshot 2 → use Comparison view → sort by #Delta to find objects that grew", "Check the Console for errors", "Use the Performance panel instead"], correct: 1, explain: "Memory panel → Heap snapshot → Take snapshot (baseline) → perform the leaking action 10+ times → Take snapshot again → switch to Comparison view → sort by #Delta (count added minus removed). Objects with positive delta that shouldn't grow are your leak candidates." },
    { q: "What are detached DOM nodes and how do you find them?", opts: ["Nodes not visible on screen", "DOM elements removed from the document but still referenced by JavaScript — GC can't collect them. Find via heap snapshot filtering 'Detached'.", "Nodes with no CSS", "Shadow DOM nodes"], correct: 1, explain: "Heap snapshot → search bar → type 'Detached'. Shows all HTMLElement objects no longer in the document tree but still reachable from JS. Click any to see the retainer chain — which JS variable is preventing GC. Fix: null out the reference when removing the element." },
    { q: "What does the Allocation instrumentation timeline show?", opts: ["Network allocation", "Records object allocations over time — bars represent allocations, blue = still alive, grey = collected. Hover a bar to see what was allocated.", "CPU usage over time", "Memory per component"], correct: 1, explain: "Memory panel → Allocation instrumentation on timeline → Record → interact → Stop. Blue bars = objects allocated but not yet GC'd. Grey bars = collected. Look for blue bars that never become grey — those objects are retained. Click blue bars to see what was created." },
    { q: "How do you use the Performance Monitor to detect a leak without taking snapshots?", opts: ["You can't detect leaks there", "More Tools → Performance Monitor → watch JS Heap Size. Healthy = sawtooth (grows then drops on GC). Leaking = continuous upward slope that never drops.", "Check the Network tab", "Performance Monitor only shows FPS"], correct: 1, explain: "Performance Monitor is a live dashboard. Open it, interact repeatedly with the suspected feature, watch JS Heap Size. Healthy pattern: heap grows during interaction, drops when GC runs (sawtooth). Leaking: heap grows and never fully returns to baseline. Confirms a leak before you commit to snapshot analysis." },
  ],

  'devtools-sources': [
    { q: "What is a conditional breakpoint and when is it useful?", opts: ["A breakpoint with a timeout", "A breakpoint that only pauses when a condition is true — right-click gutter → Add conditional breakpoint → enter JS expression", "A breakpoint in a catch block", "A breakpoint that logs instead of pausing"], correct: 1, explain: "Right-click line number → Add conditional breakpoint → type: userId === '12345' or items.length > 100. Only pauses when the condition is true. Essential in loops — without it you'd have to click Resume thousands of times to reach the one iteration you care about." },
    { q: "What is a logpoint and how does it differ from console.log?", opts: ["Same thing", "Logpoint: logs expression to Console WITHOUT pausing AND without modifying source code. Removed when DevTools closes.", "Logpoints are slower", "Logpoints only work in Chrome"], correct: 1, explain: "Sources panel → right-click line gutter → Add logpoint → enter: 'Processing item:', item.id. Logs every time that line executes. Zero code modification, not committed to git, automatically removed. Ideal for debugging without polluting source." },
    { q: "What is the Call Stack panel in Sources useful for?", opts: ["Showing network requests", "Shows the exact function call chain that led to the current breakpoint — click any frame to jump to that function's context and inspect its variables", "Showing CSS inheritance", "Measuring performance"], correct: 1, explain: "When paused at a breakpoint, Call Stack shows every function that called every function down to where you are. Click any frame to jump to that execution context — Scope panel updates to show that frame's local variables. Essential for understanding how you got somewhere." },
    { q: "How do you debug minified production code in DevTools?", opts: ["You can't debug minified code", "Sources panel → click {} (pretty print) at bottom to format minified code. With source maps: original source files appear automatically in the Sources tree.", "Use console.log only", "Rebuild without minification"], correct: 1, explain: "Pretty print ({}): formats minified code for readability — breakpoints work on the formatted version. Source maps: if your build uploads source maps, Chrome DevTools automatically shows original TypeScript/JSX source with working breakpoints, even in production builds pointing to Sentry." },
    { q: "What does 'Never pause here' do in Sources?", opts: ["Disables all breakpoints", "Right-click a breakpoint → Never pause here — tells debugger to skip this specific location even if there's an exception there. Useful for noisy third-party code.", "Removes the file from the sources tree", "Disables the file"], correct: 1, explain: "When debugging 'pause on caught exceptions', third-party libraries often throw/catch internally causing constant pauses. Right-click the noisy location → Never pause here. DevTools skips it. You can also right-click files in the Sources tree → Add to ignore list to skip all code in that file." },
  ],
};
